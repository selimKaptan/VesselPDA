...
-   **Port Call Hub**: Redesigned as the central operations hub with dedicated detail pages. Voyage detail page (`/voyages/:id`) uses a scrollable 8-tab structure: Overview (PortCallWorkflow), SOF (NOR+SOF+Port Conditions), Appointments, Cargo (Multi-Parcel Tracking + Stowage Plan + Operation Logs), Crew Ops (Husbandry/Crew Change logistics), Financials, Documents, Team. Tabs are conditionally shown based on voyage type (purposeOfCall). Includes a Port Call Checklist.
-   **Cargo Parcel Dashboard** (Cargo tab): New multi-parcel tracking system with 5-stat summary header, 2-column layout (3/5 parcel list with inline quantity update + 2/5 stowage plan upload & hold notes). Tables: `cargo_parcels` (per-parcel tracking with receiver, cargo type, target/handled quantities, hold numbers, B/L, status) and `stowage_plans` (file URL + hold notes). Routes: `GET/POST /api/voyages/:id/cargo-parcels`, `PATCH/DELETE /api/voyages/cargo-parcels/:pid`, `GET/PATCH /api/voyages/:id/stowage-plan`. Operation Logs section preserved below.
