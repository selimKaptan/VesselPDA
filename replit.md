...
- **Schema Integrity**: `shared/schema.ts` fully aligned with production DB — 24 tables corrected from `generatedAlwaysAsIdentity()` to `serial()` to match actual DB sequences; `organization_members.role_id` orphan column registered in schema; `chamber_of_shipping_fees` and `light_dues` tariff tables added to schema. Deployment schema drift issue resolved.
