...
- **Core Features**:
    ...
    - **Demo Mode**: No-signup interactive demo at `/demo`. Role selection (Agent/Shipowner/Admin), guided 4-step tour overlay, full mock dashboard with vessel list, proformas, voyages, SOF, Final DA, compliance, and bunker records. Server: `server/demo-session.ts` (in-memory sessions, 24h TTL), API endpoints `POST /api/demo/start`, `POST /api/demo/switch-role`, `POST /api/demo/reset`, `GET /api/demo/status`. Client: `client/src/lib/demo-data.ts` (static mock data), `client/src/contexts/demo-context.tsx` (DemoProvider, useDemo hook), `client/src/components/demo-banner.tsx` (persistent top banner), `client/src/pages/demo.tsx`. Cron: daily cleanup at 02:00. Landing page "Watch Demo" button links to /demo. Demo banner shows role switcher and Register CTA.
