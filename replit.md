...
- **Laytime Calculator Persistence**: `laytime_sheets` DB table (userId, voyageId FK, title, vesselName, portName, terms JSONB, events JSONB, result JSONB). Full CRUD via `GET/POST/PATCH/DELETE /api/laytime-sheets`. Frontend at `/laytime-calculator` with saved sheets sidebar panel, New/Save/Delete buttons, 1.5s debounced autosave, `?voyageId=N` auto-fill from voyage, jsPDF export. Voyage Detail Financial tab shows linked laytime sheets with "New Calculation" link. Sidebar: Scale icon for agent/shipowner.
- **DA Advance / Fund Request System**: `da_advances` DB table (userId, voyageId FK, proformaId FK, title, requestedAmount, receivedAmount, currency, status, dueDate, recipientEmail, principalName, notes, bankDetails). Status flow: pending → partially_received → fully_received → cancelled. CRUD via `GET/POST/PATCH/DELETE /api/da-advances` + `POST /api/da-advances/:id/record-payment`. Frontend at `/da-advances` with summary cards, filter chips, progress bars, record-payment dialog, create dialog. Voyage Detail Financial tab shows linked DA advances. Email notification via `sendDaAdvanceRequestEmail()` when recipientEmail set. Sidebar: Banknote icon.
- **Analytics & Reporting Dashboard**: `GET /api/analytics/overview?period=3|6|12` returns KPIs (total voyages, PDA value, FDA variance%, invoices paid/pending), monthly voyage trend, monthly invoice revenue, top ports (JOIN ports table), invoice status breakdown, voyage status breakdown, vessel activity (shipowner). Frontend at `/analytics` with Recharts: BarChart (voyages), AreaChart (revenue), horizontal BarChart (ports), PieChart (invoice status). Period toggle 3/6/12 months. Dark mode compatible. Sidebar: BarChart2 icon for all roles.
- **Global Search: Invoice + FDA Results**: `/api/search` now includes invoices (search by title, user-filtered) and FDA accounts (search by reference number, user-filtered). Frontend TYPE_ORDER and TYPE_LABELS updated. Invoice icon 💳, FDA icon 📊.
