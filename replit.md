...
- **AI Smart Drop Module**: Drag-and-drop / paste document analysis using Claude claude-haiku-4-5. Detects 11 maritime event types (crew_change, eta_update, nor_tendered, etc.). Glassmorphism drop zone component (`client/src/components/ai-smart-drop.tsx`). Analysis result modal with edit mode and confirm & process routing. 5/min rate limiter. DB table `ai_analysis_history`. Endpoints: POST /api/ai/parse-document, POST /api/ai/execute-action, GET /api/ai/history. Dashboard widget (AiSmartDropMini) on all 5 role dashboards. Full page at /ai-smart-drop under Tools navigation.
