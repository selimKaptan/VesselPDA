...
-   **F3 – Modular Schema**: `shared/schema.ts` (3205 satır) bölündü → `shared/schema/` dizini altında 19 etki alanı modülü. Döngüsel bağımlılıklar çözüldü: `voyage.ts`'deki `many(proformas)` ve `many(serviceRequests)` back-relation'ları kaldırıldı (forward reference yeterliydi). `shared/schema.ts` artık tek satırlık `export * from "./schema/index"`. Modüller: `vessel`, `port`, `forum`, `organization`, `tender`, `voyage`, `proforma`, `finance`, `service`, `messaging`, `crew`, `environmental`, `insurance`, `commercial`, `maintenance`, `drydock`, `husbandry`, `tariffs`, `misc`.