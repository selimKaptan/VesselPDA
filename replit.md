...
- **Core Features**:
    ...
    - **Organization & Team Management**: Multi-user workspace for maritime companies. Uses the existing `organizations`, `organization_members`, and `organization_invites` DB tables (not new ones). Schema-mapped to actual DB columns: `organizations.type` (industry), `organization_members.role` (orgRole), `organization_members.job_title` (title), `organization_invites.invited_email` and `invited_by_user_id`. Features: create-org flow for new users, Members tab (table with role badges: owner=amber/admin=sky/manager=purple/member=slate/viewer=gray, deactivate/remove actions), Invitations tab (send by email with 7-day token, resend/cancel), Permissions matrix tab (8 permissions × member rows, auto-saves on checkbox toggle). Plan-based member limits: free=1, standard=5, unlimited=50. `orgInviteTemplate()` email in `server/email-templates.ts`, `sendOrgInviteEmail()` in `server/email.ts`. Backend: `server/routes/organization.routes.ts` with `orgRouter` (mounted `/api/organizations`) and `inviteRouter` (mounted `/api/invites`). Page: `client/src/pages/team.tsx`. Settings sidebar nav: "Team Management" → `/team`.
    ...
    - **DA Comparison Report Module**: Professional PDA vs FDA line-item variance analysis report. List mode (`/da-comparison`) shows historical accuracy table with average accuracy gauge (circular SVG ring). Detail mode (`/da-comparison/:proformaId`) shows 4-card summary grid (estimated/actual/variance/accuracy), color-coded line-item table, horizontal Recharts BarChart with ReferenceLine at 0, top over/under insights panel, and suggestions. PDF export via PDFKit (`GET /api/da-comparison/:proformaId/pdf`). Backend: `server/routes/da-comparison.routes.ts` with `/history` and `/:proformaId` endpoints using medium-tier cache. Integration: "DA Comparison" button on proforma detail when FDA exists; "Compare with PDA" button on FDA detail; accuracy widget on agent dashboard; Finance sidebar entry.
    - **Vessel Schedule Calendar**: Fleet schedule management with three views (Month calendar with event pills, Gantt-style Timeline with zoom controls, and List table). Vessel filter dropdown, Quick Add Port Call dialog (vessel/port/operation/ETA/ETD). Event popovers with doc status badges (NOR/SOF/PDA/FDA). `UpcomingPortCallsWidget` on shipowner and agent dashboards showing countdown (Soon/Today/Tomorrow/N days). Backend: `server/routes/vessel-schedule.routes.ts` with `GET /api/vessel-schedule` (events + vessels with existence checks) and `GET /api/vessel-schedule/upcoming`. No new DB tables — queries from existing `voyages` table. Fleet sidebar navigation entry at `/vessel-schedule`.
...
- **Yahoo Finance API**: For market data indices (BDI, BCTI, BDTI).
    - **Voyage Activity Timeline**: `voyage_activities` DB table tracks all voyage events. `server/voyage-activity.ts` helper (`logVoyageActivity()`) fires-and-forgets from 15+ endpoints across voyage, SOF, FDA, invoice, tender routes. Voyage-detail has a 5th "Activity" tab with vertical timeline (colored by event type, `formatTimeAgo` timestamps, Add Note dialog). `GET /api/voyages/:id/activities` returns paginated timeline. `GET /api/user/recent-activity` powers a "Recent Activity" card on all 4 role dashboards. Activity types: voyage_created, status_changed, eta_updated, document_uploaded, document_signed, checklist_added, checklist_completed, chat_message, sof_created, sof_finalized, fda_created, fda_approved, invoice_created, invoice_paid, nomination_sent, review_submitted, custom_note.
    ...
    - **Email Template Engine**: Centralized `server/email-templates.ts` with 18 template functions using a shared `baseTemplate()` dark maritime theme (body #0B1120, card #1a2744, sky-blue #38BDF8 CTAs, inline+stylesheet CSS for email client compatibility). All 12 `send*` functions in `server/email.ts` now use templates instead of inline HTML. Templates: `pdaSentTemplate`, `pdaFullTemplate`, `pdaApprovalRequestTemplate`, `newTenderTemplate`, `bidReceivedTemplate`, `bidSelectedTemplate`, `nominationTemplate`, `nominationResponseTemplate`, `norTenderedTemplate`, `invoiceCreatedTemplate`, `fdaReadyTemplate`, `welcomeTemplate`, `verificationTemplate`, `passwordResetTemplate`, `forumReplyTemplate`, `messageBridgeTemplate`, `contactAdminTemplate`, `contactAutoReplyTemplate`. Admin can preview all templates via `GET /api/admin/email-preview/:template` — accessible via the "Email Templates" card in System Settings.