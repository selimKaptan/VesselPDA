...
- **Core Features**:
    - **User & Role Management**: 5-role system — `shipowner`, `agent`, `broker`, `provider`, `admin` — stored in `userRole` field. Register page uses 2-step flow: Step 1 = 4 role selection cards (Ship Agent / Shipowner / Ship Broker / Ship Provider), Step 2 = registration form. Each role has a distinct dashboard and role-based navigation. `server/middleware/role-guard.ts` provides `requireRole(...roles)` middleware; applied via `app.use` to `/api/admin/*` (admin only), `/api/fixtures/*` (shipowner/broker/admin), `/api/service-offers/*` (provider/admin).
    ...
    - **Navigation System**: Top-bar layout replacing the old sidebar. Top bar: Logo (left) + Module tabs (center, role-based) + Lang/Dark/Notifications/User (right). Left side panel (200px, desktop only) shows active module's sub-pages. Mobile: sheet-based hamburger drawer with expandable module sections. Layout: `client/src/components/layout/app-layout.tsx`. Role-navigation config: `client/src/lib/role-navigation.ts` with `getModulesForRole(role)` and `getActiveModule()`. SidebarProvider wrapper preserved for backward compat with useSidebar() hooks in vessels.tsx and proformas.tsx.
    ...
    - **Audit Log System**: `audit_logs` DB table (raw SQL); `server/audit.ts` with `logAction(userId, action, entityType, entityId, details, ipAddress)` — async, non-blocking, never fails the parent operation. Logs: user login, proforma create/delete/duplicate, DA approval, tender create/cancel, bid submit/select, voyage create/status-change, invoice create/pay/cancel. Admin endpoint: `GET /api/admin/audit-logs` with query filters (userId, action, entityType, from, to, limit, offset).
    ...
    - **Fixture & Recap System**: System for charter negotiation and fixture tracking, with integrated **Laytime & Demurrage Calculator** — per-port-call calculations (loading/discharging), NOR/berthing/departure timeline inputs, configurable deductions (Sundays, holidays, weather), live preview card, automatic demurrage/despatch amounts using `server/laytime-calculator.ts`; summary strip (Total Demurrage, Total Despatch, Net Balance); `laytime_calculations` DB table; endpoints: GET/POST `/api/fixtures/:id/laytime`, PUT/DELETE `/api/laytime/:id`.
