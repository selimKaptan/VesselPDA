...
## Recent Changes
- 2026-02-26: Live AIS tracking via AISStream.io — WebSocket connection to wss://stream.aisstream.io/v0/stream on server startup. Subscribes to Turkish waters bounding box [35.0-42.5°N, 25.0-45.0°E]. Caches positions in memory Map<mmsi, VesselPosition>. Falls back to mock data if no key. New file: server/ais-stream.ts. New endpoint: GET /api/vessel-track/status (returns connected/vesselCount/mode). Frontend badge shows green pulsing "LIVE AIS" when connected, amber "DEMO" otherwise. Auto-refresh: 15s live, 60s demo. AIS_STREAM_API_KEY = aisstream.io free account key.
- 2026-02-26: Vessel IMO auto-lookup switched to RapidAPI (Zyla Labs Vessel Information API). Endpoint: GET /1498/get%2Bvessel%2Binfo?imoCode={IMO}. Returns vessel_name, ship_type, flag (full name), gross_tonnage, length_overall_m, beam_m. VESSEL_API_KEY = RapidAPI subscription key.
- 2026-02-26: Live TCMB exchange rates on /proformas/new — "Fetch Live Rates (TCMB)" button auto-fills USD/TRY and EUR/TRY from Turkish Central Bank's public XML feed. New public endpoint GET /api/exchange-rates fetches and parses TCMB XML, returns mid-rate (avg of buying/selling). Shows date and values in success toast. Manual override still possible.
- 2026-02-26: Hierarchical port selection on /proformas/new — replaced flat 804-port combobox with a 2-step City/Region → Terminal selector. City dropdown shows 63 Turkish cities (derived from LOCODE port codes, e.g. TRMER → Mersin) sorted alphabetically with terminal count. Terminal combobox is disabled until a city is chosen, then filters to that city's ports only with inline search. Port summary row appears below showing City, Terminal name, and LOCODE code. No DB changes — purely frontend using CITY_CODE_NAMES lookup map.
