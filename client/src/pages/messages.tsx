        ...
        <div className="space-y-2">
          {filtered.map((conv: any) => (
            <Link key={conv.id} href={`/messages/${conv.id}`}>
              <div
                className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-card border hover:border-[hsl(var(--maritime-primary)/0.4)] hover:shadow-sm transition-all cursor-pointer"
                data-testid={`conversation-${conv.id}`}
              >
                <div className="w-10 h-10 rounded-full bg-[hsl(var(--maritime-primary)/0.12)] flex items-center justify-center flex-shrink-0 font-bold text-sm text-[hsl(var(--maritime-primary))]">
