...
export default function LaytimeCalculator() {
  ...
  const setTerm = useCallback(
    <K extends keyof Terms>(key: K, value: Terms[K]) =>
  ...
  const addEvent = useCallback(() => {
    setEvents((prev) => [
      ...prev,
      { id: uid(), dateTime: "", description: "", toCount: 100 },
    ]);
  }, []);
  ...
  const updateEvent = useCallback(
    <K extends keyof SofEvent>(id: string, key: K, value: SofEvent[K]) => {
      setEvents((prev) =>
  ...
  const calc = useMemo<CalcResult>(() => {
    const allowed =
      (parseFloat(terms.allowedDays) || 0) * 24 +
      (parseFloat(terms.allowedHours) || 0);
    ...
    if (allowed > 0) {
      if (diff > 0) {
        demurrageAmount = diff * (demRate / 24);
        status = "on_demurrage";
      } else if (diff < 0) {
        despatchAmount = Math.abs(diff) * (desRate / 24);
        status = "on_despatch";
      }
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Charter party terms &amp; statement of facts — live demurrage / despatch
              </p>
            </div>
          </div>
          <Button
      ...
      <div className="flex gap-6 p-6 items-start">
        ...
        <div className="flex-1 flex flex-col gap-5 min-w-0">
          ...
          <div
            ...
            data-testid="card-charter-party-terms"
          >
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    min="0"
                    placeholder="e.g. 4"
                    value={terms.allowedDays}
                    onChange={(e) => setTerm("allowedDays", e.target.value)}
                    className="h-9"
                    data-testid="input-allowed-days"
                  />
                </div>

                {/* Allowed Time — Extra Hours */}
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTerm("allowedHours", e.target.value)}
                    className="h-9"
                    data-testid="input-allowed-hours"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTerm("demurrageRate", e.target.value)}
                    className="h-9"
                    data-testid="input-demurrage-rate"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTerm("despatchRate", e.target.value)}
                    className="h-9"
                    data-testid="input-despatch-rate"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={terms.laytimeTerms}
                    onValueChange={(v) => setTerm("laytimeTerms", v)}
                  >
                    <SelectTrigger className="h-9" data-testid="select-laytime-terms">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setTerm("currency", v)}
                  >
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-border bg-muted/20">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-1 text-[10px] text-muted-foreground bg-muted rounded-full px-2 py-0.5">
              ...
              <Button
                ...
                data-testid="button-add-sof-event"
              >
                ...
                <tbody>
                  {events.map((ev, idx) => (
                    <tr
                      ...
                      data-testid={`row-sof-event-${idx}`}
                    >
                      {/* # */}
                      <td className="px-4 py-2 text-[11px] text-muted-foreground/50 font-mono">
                        {idx + 1}
                      </td>
                      ...
                      <td className="px-4 py-2">
                        <input
                          ...
                          value={ev.dateTime}
                          onChange={(e) =>
                            updateEvent(ev.id, "dateTime", e.target.value)
                          }
                          className="bg-transparent border-0 text-xs text-foreground focus:outline-none w-48 [color-scheme:dark]"
                          data-testid={`input-event-datetime-${idx}`}
                        />
                      </td>
                      ...
                      <td className="px-4 py-2">
                        <input
                          ...
                          placeholder="e.g. NOR Tendered, Rain Commenced, Loading Completed…"
                          className="bg-transparent border-0 text-xs text-foreground focus:outline-none w-full placeholder:text-muted-foreground/40"
                          data-testid={`input-event-description-${idx}`}
                        />
                      </td>
                      ...
                      <td className="px-4 py-2 text-center">
                        ...
                        >
                          <SelectTrigger
                            className="h-6 text-xs w-20 mx-auto border-border/60"
                            data-testid={`select-to-count-${idx}`}
                          >
                            <SelectValue>
                              <ToCountBadge value={ev.toCount} />
                            </SelectValue>
                          </SelectTrigger>
                          <SelectContent>
            ...
            <div className="px-5 py-3 border-t border-border/50 bg-muted/10">
              <p className="text-[10px] text-muted-foreground leading-relaxed">
                <strong>How it works:</strong> Each row marks the <em>start</em> of a laytime period.
                The "To Count %" determines how much of that period counts toward used laytime
                (100% = full count, 50% = half, 0% = excluded e.g. rain/Sundays).
                The period ends when the next event begins.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="w-80 shrink-0">
          <div className="sticky top-6 flex flex-col gap-4">
            ...
            <div
              className="rounded-xl border border-border bg-card overflow-hidden"
              data-testid="card-calculation-summary"
            >
              ...
              <div className="p-4 space-y-2.5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono text-sm font-semibold" data-testid="text-time-allowed">
                    {calc.allowed > 0 ? fmtHours(calc.allowed) : "—"}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-muted-foreground">Time Used</span>
                  <span className="font-mono text-sm font-semibold" data-testid="text-time-used">
                    {calc.timeUsed > 0 ? fmtHours(calc.timeUsed) : "—"}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs text-muted-foreground">
                    {calc.diff > 0 ? "Time Exceeded" : calc.diff < 0 ? "Time Saved" : "Balance"}
                  </span>
                  <span
                  ...
                  <div className="pt-1">
                    <div className="h-2 rounded-full bg-muted overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, (calc.timeUsed / calc.allowed) * 100)}%`,
                        }}
                      />
                    </div>
                    <p className="text-[10px] text-muted-foreground mt-1 text-right">
                      {Math.round((calc.timeUsed / calc.allowed) * 100)}% of allowed time used
                    </p>
                  </div>
                )}
              </div>
            ...
            {calc.periods.length > 0 && (
              <div
                className="rounded-xl border border-border bg-card overflow-hidden"
                data-testid="card-time-breakdown"
              >
                <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-muted/20">
                ...
                <div className="p-4 space-y-3">
                    ...
                    {calc.periods.map((p, i) => {
                      ...
                      return (
                        <div
                          ...
                          title={`${p.description || "Period"} — ${fmtHours(p.hours)} raw · ${fmtHours(p.effective)} counted (${p.toCount}%)`}
                        />
                      );
                    })}
                  </div>
                  ...
                  <div className="pt-1 border-t border-border/50 flex justify-between text-[11px]">
                    <span className="text-muted-foreground">
                      Total span: {fmtHours(calc.totalSpan)}
                    </span>
                    <span className="font-semibold">
                      Counted: {fmtHours(calc.timeUsed)}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div
              ...
              data-testid="card-financial-result"
            >
              <div className="p-5 text-center space-y-2">
                ...
                {calc.status === "on_demurrage" && (
                  <>
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <p
                        className="text-3xl font-black text-red-500 tabular-nums"
                        data-testid="text-financial-amount"
                      >
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {fmtHours(calc.diff)} over laytime ×{" "}
                      {fmtMoney(parseFloat(terms.demurrageRate) || 0, terms.currency)}/day
                    </p>
                  </>
                )}
                ...
                {calc.status === "on_despatch" && (
                  <>
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <p
                        className="text-3xl font-black text-emerald-500 tabular-nums"
                        data-testid="text-financial-amount"
                      >
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {fmtHours(Math.abs(calc.diff))} saved ×{" "}
                      {fmtMoney(parseFloat(terms.despatchRate) || 0, terms.currency)}/day
                    </p>
                  </>
                )}
                ...
                {calc.status === "within_laytime" && (
                  <>
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <p
                        className="text-3xl font-black text-blue-500 tabular-nums"
                        data-testid="text-financial-amount"
                      >
                        {terms.currency === "USD" ? "$" : terms.currency === "EUR" ? "€" : "£"}0.00
                      </p>
                    </div>
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {calc.allowed > 0 && calc.timeUsed > 0
                        ? `${fmtHours(Math.abs(calc.diff))} to spare`
                        : "Enter terms & events to calculate"}
                    </p>
                  </>
                )}
                ...
                {calc.allowed > 0 && (
                  <div className="pt-2 border-t border-border/30">
                    <p className="text-[10px] text-muted-foreground">
                      {terms.laytimeTerms} · Allowed{" "}
                      {fmtHours(calc.allowed)} ·{" "}
                      {fmtMoney(parseFloat(terms.demurrageRate) || 0, terms.currency)}/day DEM /{" "}
                      {fmtMoney(parseFloat(terms.despatchRate) || 0, terms.currency)}/day DES
                    </p>
                  </div>
                )}
              </div>
