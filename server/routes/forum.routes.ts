...
import { isAdmin, forumTopicBodySchema, forumReplyBodySchema } from "./shared";
...
router.get("/topics", async (req: any, res: any, next: any) => {
  try {
...
router.post("/topics", isAuthenticated, async (req: any, res: any, next: any) => {
  try {
...
router.delete("/topics/:id", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.post("/topics/:id/replies", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.post("/topics/:id/like", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const topicId = parseInt(req.params.id);
...
router.post("/replies/:id/like", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.get("/my-likes", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.post("/topics/:id/dislike", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const topicId = parseInt(req.params.id);
    if (!topicId) return res.status(400).json({ message: "Invalid topic ID" });
...
router.post("/replies/:id/dislike", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const replyId = parseInt(req.params.id);
    if (!replyId) return res.status(400).json({ message: "Invalid reply ID" });
