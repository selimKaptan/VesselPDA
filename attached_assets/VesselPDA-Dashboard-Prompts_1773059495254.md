# VesselPDA — Dashboard Prompt'ları (Replit Build Mode)

Entegrasyon prompt'larından SONRA uygulayın. Dashboard tüm modülleri birleştirir.

---

## ⭐ PROMPT D1 — Ana Dashboard API (Backend)

```
Kullanıcı giriş yaptığında tüm kritik verileri tek seferde görmeli. Role bazlı dashboard API oluştur.

server/routes/dashboard.routes.ts dosyası oluştur:
...
import { eq, and, or, desc, sql, count, gte, lte, isNull, isNotNull } from "drizzle-orm";

const router = Router();

// ─── SHIPOWNER / OPERATOR DASHBOARD ──────────────────────────────────
router.get("/", isAuthenticated, async (req, res) => {
  const userId = (req as any).user?.id;
  const role = (req as any).user?.activeRole || (req as any).user?.userRole || "shipowner";
  ...
  try {
    ...
    // ─── FLEET OVERVIEW ────────────────────────────────────────────
    const userVessels = await db.select().from(vessels)
    ...
    // ─── ACTIVE VOYAGES ────────────────────────────────────────────
    let activeVoyagesQuery;
    if (role === "agent") {
      activeVoyagesQuery = db.select({
        ...
        .where(and(eq(voyages.agentUserId, userId), or(eq(voyages.status, "planned"), eq(voyages.status, "in_progress"), eq(voyages.status, "active"))))
        .orderBy(voyages.eta)
        .limit(10);
    } else {
      activeVoyagesQuery = db.select({
        ...
        .where(and(eq(voyages.userId, userId), or(eq(voyages.status, "planned"), eq(voyages.status, "in_progress"), eq(voyages.status, "active"))))
        .orderBy(voyages.eta)
        .limit(10);
    }
    const activeVoyages = await activeVoyagesQuery;
    ...
    if (role === "agent") {
      ...
      const activeBids = myBids.filter(b => b.tenderStatus === "open");
      const wonBids = myBids.filter(b => b.status === "selected");
      
      tenderData = {
        activeBidsCount: activeBids.length,
        wonBidsCount: wonBids.length,
        totalBidsCount: myBids.length,
      };
      nominationData = {
        pendingCount: pendingNominations,
      };
    }
    ...
    if (role === "shipowner") {
      ...
      myTenders = tenders[0] || { total: 0, open: 0, nominated: 0 };
    }

    // ─── BROKER-SPECIFIC ──────────────────────────────────────────
    let brokerData = null;
    if (role === "broker") {
      const fixtureStats = await db.select({
        total: sql<number>`COUNT(*)::int`,
    ...
    // ─── NOTIFICATIONS (son 10 okunmamış) ──────────────────────────
    ...
    // ─── EXPENSE BY CATEGORY (son 30 gün, pie chart için) ─────────
    ...
    // ─── PDA vs FDA VARIANCE TREND (son 10 FDA) ────────────────────
    ...
    res.json({
      ...
      alerts: {
        ...
        maintenanceAlerts,
        upcomingPortCalls,
        overdueInvoices: (invoiceStats[0] as any)?.overdueCount || 0,
      },
...
Sonra server/routes.ts'de bu route'u ekle:
import dashboardRoutes from "./routes/dashboard.routes";

// Route registration bölümüne:
app.use("/api/dashboard", isAuthenticated, dashboardRoutes);
```

---

## ⭐ PROMPT D2 — Fleet Map & Status Dashboard Widget

```
Dashboard'da gemilerin harita üzerinde konumlarını ve durumlarını gösteren bir fleet overview endpoint'i ekle.

server/routes/dashboard.routes.ts dosyasına yeni endpoint ekle:

// GET /api/dashboard/fleet-map — Tüm gemilerin son pozisyonları
router.get("/fleet-map", isAuthenticated, async (req, res) => {
  const userId = (req as any).user?.id;
  ...
  try {
    ...
    const fleetPositions = await Promise.all(userVessels.map(async (vessel) => {
      ...
      if (vessel.mmsi) {
        ...
        if (lastPos) {
          position = {
            ...
            destination: lastPos.destination,
      ...
      // AIS yoksa noon report'tan pozisyon al
      if (!position) {
        const [lastNoon] = await db.select({
          latitude: noonReports.latitude,
          longitude: noonReports.longitude,
          speedOverGround: noonReports.speedOverGround,
          reportDate: noonReports.reportDate,
        }).from(noonReports)
        ...
        if (lastNoon?.latitude && lastNoon?.longitude) {
          position = {
            latitude: lastNoon.latitude,
            longitude: lastNoon.longitude,
            speed: lastNoon.speedOverGround,
            lastUpdate: lastNoon.reportDate,
            source: "noon_report",
          };
        }
      }
      
      // Aktif voyage bilgisi
...
## ⭐ PROMPT D3 — Voyage Timeline Dashboard

```
Her voyage için zaman çizelgesini gösteren endpoint. Tüm olayları kronolojik sırala.

server/routes/dashboard.routes.ts dosyasına ekle:

// GET /api/dashboard/voyage/:id/timeline — Voyage timeline
router.get("/voyage/:id/timeline", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    for (const pc of portCallList) {
      ...
      if (pc.actualArrival) timeline.push({ timestamp: pc.actualArrival, type: "arrival", title: "Vessel Arrived", description: pc.portName });
      if (pc.norTendered) timeline.push({ timestamp: pc.norTendered, type: "nor", title: "NOR Tendered", description: pc.portName });
      if (pc.berthingTime) timeline.push({ timestamp: pc.berthingTime, type: "berthing", title: "Vessel Berthed", description: pc.berth || pc.portName });
      if (pc.operationsStart) timeline.push({ timestamp: pc.operationsStart, type: "ops_start", title: "Operations Started", description: `${pc.cargoType || "Cargo"} ${pc.cargoQuantity ? pc.cargoQuantity + " MT" : ""}` });
      if (pc.operationsEnd) timeline.push({ timestamp: pc.operationsEnd, type: "ops_end", title: "Operations Completed" });
      if (pc.departure) timeline.push({ timestamp: pc.departure, type: "departure", title: "Vessel Departed", description: pc.portName });
    }
    ...
    for (const nor of nors) {
      ...
      if (nor.norAcceptedAt) timeline.push({ timestamp: nor.norAcceptedAt, type: "nor_accepted", title: "NOR Accepted", description: `By: ${nor.norAcceptedBy || ""}`, status: nor.status });
    }
    ...
    for (const pda of proformaList) {
      ...
      if (pda.sentAt) timeline.push({ timestamp: pda.sentAt, type: "proforma_sent", title: `PDA Sent: ${pda.referenceNumber}` });
      if (pda.reviewedAt) timeline.push({ timestamp: pda.reviewedAt, type: "proforma_reviewed", title: `PDA ${pda.approvalStatus}: ${pda.referenceNumber}`, status: pda.approvalStatus });
    }
    ...
    for (const inv of invoiceList) {
      ...
      if (inv.paidAt) timeline.push({ timestamp: inv.paidAt, type: "payment", title: `Payment Received: ${inv.title}`, description: `$${inv.amount?.toLocaleString() || 0}` });
    }
    ...
    // 10. Voyage notes
    const notesList = await db.select().from(voyageNotes)
      .where(eq(voyageNotes.voyageId, voyageId));
...
## ⭐ PROMPT D4 — KPI Analytics Endpoint

```
Performans göstergeleri (KPI) endpoint'i oluştur. Bu veriler dashboard kartlarında ve grafiklerde kullanılacak.

server/routes/dashboard.routes.ts dosyasına ekle:

// GET /api/dashboard/kpi — Key Performance Indicators
router.get("/kpi", isAuthenticated, async (req, res) => {
  const userId = (req as any).user?.id;
  const period = (req.query.period as string) || "30d"; // 7d, 30d, 90d, 365d
  
  const periodDays = period === "7d" ? 7 : period === "90d" ? 90 : period === "365d" ? 365 : 30;
  const since = new Date(Date.now() - periodDays * 24 * 60 * 60 * 1000);
  
  try {
    // 1. PDA Accuracy — PDA vs FDA varyans ortalaması
    ...
    const accuracyRate = accuracy.fda_count > 0 
      ? Math.round((accuracy.accurate_count / accuracy.fda_count) * 100) 
      : 0;

    // 2. Average Port Stay — Ortalama liman kalış süresi
    ...
    // 3. Collection Rate — Ödeme tahsilat oranı
    ...
    const collectionRate = collection.total_invoiced > 0 
      ? Math.round((collection.total_collected / collection.total_invoiced) * 100) 
      : 0;

    // 4. Vessel Utilization — Gemi kullanım oranı
    const vesselUtilization = await db.execute(sql`
      SELECT 
        ...
        COALESCE(SUM(
          ...
          THEN EXTRACT(EPOCH FROM (voy.etd - voy.eta)) / 86400 
          END
    ...
    // 5. Top Ports — En çok gidilen limanlar
    ...
    // 6. Expense Trend — Günlük/haftalık harcama trendi
...
| Sıra | Prompt | Ne Sağlıyor |
|------|--------|-------------|
| 1 | D1 | Ana dashboard — kullanıcı giriş yaptığında her şeyi görür |
| 2 | D4 | KPI analytics — grafikler ve performans kartları |
| 3 | D3 | Voyage timeline — seferin tüm hikayesi kronolojik |
| 4 | D2 | Fleet map — gemilerin haritadaki pozisyonları |

NOT: Bu prompt'lar sadece BACKEND API oluşturur. Frontend (React) tarafında bu endpoint'leri çağıran dashboard sayfaları da oluşturulmalı. Bunu ayrı bir prompt olarak verin:

"Dashboard sayfasını oluştur. /api/dashboard endpoint'inden veri çek ve şu widget'ları göster:
1. Fleet Overview kartı (gemi sayısı, durum dağılımı)
2. Active Voyages listesi
3. Financial Summary (invoiced, paid, pending, overdue)
4. Certificate Alerts
5. PDA vs FDA Variance chart (recharts)
6. Monthly Revenue trend chart
7. Expense by Category pie chart
8. Upcoming Port Calls
9. Unread notifications badge
10. Quick actions (New Proforma, New Voyage, etc.)"
```
